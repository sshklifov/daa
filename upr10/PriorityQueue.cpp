...
static void siftDown(PriorityQueue& q, int i)
{
    ...
    int smallest = i;
    if (l < q.h.size() && q.h[l].w < q.h[i].w) smallest = l;
    if (r < q.h.size() && q.h[r].w < q.h[smallest].w) smallest = r;

    if (smallest != i)
    {
        std::swap(q.h[i], q.h[smallest]);
        std::swap(q.pos[q.h[i].to], q.pos[q.h[smallest].to]);
        siftDown(q, smallest);
    }
}
...
const Edge& heapMinimum(const PriorityQueue& q)
{
    return q.h[0];
}
...
void buildMinHeap(PriorityQueue& q, const std::vector<int>& v)
{
    int n = v.size();
...
void heapDecreaseKey(PriorityQueue& q, int v, int key)
{
    int pos = q.pos[v];
